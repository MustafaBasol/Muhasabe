...
export default function SimpleSalesPage({ customers = [], sales = [], invoices = [], onSalesUpdate, onCreateInvoice, onViewInvoice, onEditInvoice, onDownloadSale }: SimpleSalesPageProps) {
  ...
  const handleConfirmCreateInvoice = () => {
    if (selectedSaleForInvoice) {
      const invoiceData = {
        ...
        customerEmail: selectedSaleForInvoice.customerEmail || '',
        customerAddress: '',
        ...
        items: [{
          id: '1',
          description: selectedSaleForInvoice.productName,
          quantity: selectedSaleForInvoice.quantity || 1,
          unitPrice: selectedSaleForInvoice.unitPrice || selectedSaleForInvoice.amount,
          total: selectedSaleForInvoice.amount
        }],
        subtotal: selectedSaleForInvoice.amount / 1.18,
        taxAmount: selectedSaleForInvoice.amount - (selectedSaleForInvoice.amount / 1.18),
        total: selectedSaleForInvoice.amount,
        notes: `Bu fatura ${selectedSaleForInvoice.saleNumber || `SAL-${selectedSaleForInvoice.id}`} numaralı satıştan oluşturulmuştur.`
      };
      ...
      // Show the created invoice in modal
